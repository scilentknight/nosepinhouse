    ...
    <div>
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500">Categories are synced from OMS and shown here for reference.</p>
        </div>
        <div className="flex gap-2">
          {false && <Link href="/admin/categories/trash">
            <Button variant="adminOutline" size="sm">Trash</Button>
          </Link>}
          {false && can("categories.create") && (
            ...
            <ul className="divide-y divide-gray-100 md:hidden">
              {rows.map((c) => (
                <li key={c.id} className="flex items-center gap-3 px-4 py-3">
                  {false && <input type="checkbox" checked={selectedIds.has(c.id)} onChange={() => toggleSelect(c.id)} className="h-4 w-4 shrink-0 rounded border-gray-300" />}
                  {can("categories.edit") ? (
